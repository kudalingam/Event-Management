...
if(isset($_POST['enquiry'])){
    ...
    if(isset($_POST['Send'])){
            ...
            if(mysqli_connect_error()){
                die('Connect Error ('.mysqli_connect_error().')'.mysqli_connect_error());
                    ...
                    if($conn->query($sql)=== TRUE){
                            ob_clean();
                            echo "New record is inserted successfully";
                            header("Location:http://localhost/login/contact.php ");
                            exit();
                    }
                    else{
                           echo "Error: ".$sql ."<br>".$conn->error;
           ...
           $image = $_FILES['image']['tmp_name']; 
               $photo = addslashes(file_get_contents($image)); 
                        ...
                        if($conn->query($sql)=== TRUE){
                                ob_clean();
                                echo "New record is inserted successfully";
                                header("Location:http://localhost/login/form.html");
                                exit();
                        }
                        else{
                               echo "Error: ".$sql ."<br>".$conn->error;
        ...
        /*$user_check_query = "SELECT * FROM sign WHERE Email='$email' And Password='$pass' LIMIT 1";
        $result = mysqli_query($conn, $user_check_query);
        $user = mysqli_fetch_assoc($result);
        $query=mysqli_query($conn,"select * from sign where Email='$email' and Password='$pass'");
        $rowcount=mysqli_num_rows($query);
    if ($result == 1){
...
$result=mysqli_query("select * from sign where Email=$email and Password=$pass");
